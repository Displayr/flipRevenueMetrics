...
test_that("Recurring Revenue",
          {
              ...
              # Matching to yearly
              m <- c(match(as.character(AsDate(names(sy)[-1])), names(s)) - 1, length(s))
              expect_equal(as.numeric(sy), as.numeric(s[m]))
              # Checking that daily data matches yearly data
              by = "month"
              s = RevenueMetric("RecurringRevenue", output = "Table",  d$AUD, d$ValidFrom, d$ValidTo, id = d$name,  by = by)
              # Matching to yearly
              m <- c(match(AsDate(names(sy)[-1]), AsDate(names(s))) - 1, length(s))
              expect_equal(as.numeric(sy), as.numeric(s[m]))
          }
)
